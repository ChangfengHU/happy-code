...
export class MessageQueue2<T, M = string> {
    ...
    private batchCombiner: (messages: M[]) => M;
    modeHasher: (mode: T) => string;

    constructor(
        modeHasher: (mode: T) => string,
        onMessageHandler: ((message: M, mode: T) => void) | null = null,
        batchCombiner?: (messages: M[]) => M
    ) {
        this.modeHasher = modeHasher;
        this.onMessageHandler = onMessageHandler;
        this.batchCombiner = batchCombiner ?? ((messages: M[]) => messages.join('\n') as M);
    ...
    private collectBatch(): { message: M, mode: T, hash: string, isolate: boolean } | null {
        ...
        const combinedMessage = this.batchCombiner(sameModeMessages);
